...
public:
    ...
    bcell_t get(bsize_t i, bsize_t j) const {
        return BOARD_VAL((bsize_t(heng[HENG(i,j,size,offset)*2+1])<<1)+bsize_t(heng[HENG(i,j,size,offset)*2]));
    }
    //绝对坐标
    ...
    bcell_t get_real(bsize_t i, bsize_t j) const {
        return BOARD_VAL((bsize_t(heng[HENG_REAL(i,j,size,offset)*2+1])<<1)+bsize_t(heng[HENG_REAL(i,j,size,offset)*2]));
    }
    vector_type<bcell_t> get_real_all(bsize_t i, bsize_t j) const {
        vector_type<bcell_t> ret = {
            BOARD_VAL((bsize_t(heng[HENG_REAL(i,j,size,offset)*2+1])<<1)+bsize_t(heng[HENG_REAL(i,j,size,offset)*2])),
            BOARD_VAL((bsize_t(shu[SHU_REAL(i,j,size,offset)*2+1])<<1)+bsize_t(shu[SHU_REAL(i,j,size,offset)*2])),
            BOARD_VAL((bsize_t(pie[PIE_REAL(i,j,size,offset)*2+1])<<1)+bsize_t(pie[PIE_REAL(i,j,size,offset)*2])),
            BOARD_VAL((bsize_t(na[NA_REAL(i,j,size,offset)*2+1])<<1)+bsize_t(na[NA_REAL(i,j,size,offset)*2])),
        };
        return ret;
    }
public:
...
public:
    ...
    class equal_func {
    public:
        uint64_t operator()(const GomokuBoard<size, offset> &b, const GomokuBoard<size, offset> &c) const {
...
public:
    ...
    GomokuBoard<size, offset>& Uodo() {
        ...
        hashval ^= zobrist[doChain.back().i][doChain.back().j][doChain.back().v];
        doChain.pop_back();
        return *this;
    }
    bcell_t Get(bsize_t i, bsize_t j) const{
    ...
    //得到点附近的棋型
    point_pattern GetPattern(bsize_t i, bsize_t j) {
        return {};
    }
public:
    GomokuBoard() {
