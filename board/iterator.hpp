...
using NextFunc = std::tuple<int, int> (*)(const int i, const int j,
                                          const int n);
static NextFunc Nexts[] = {HENG_NEXT, SHU_NEXT, PIE_NEXT, NA_NEXT};

//将横向坐标映射到特定方向的数组上, tuple(0)代表行，tuple(1)代表列
inline std::tuple<int, int> HENG_MAPING(const int i, const int j, const int size) {
    return {i, j};
}
inline std::tuple<int, int> SHU_MAPING(const int i, const int j, const int size) {
    return {j, i};
}
...
using MappingFunc = std::tuple<int, int> (*)(const int i, const int j,
                                          const int size);
