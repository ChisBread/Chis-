...
class GomokuBitBoard {
   public:  // required method
    //相对坐标
    void Set(int i, int j, const BOARD_VAL val) { SetReal(i + offset, j + offset, val); }
    void Reset(int i, int j) { ResetReal(i + offset, j + offset); }
    BOARD_VAL Get(int i, int j) const { return GetReal(i + offset, j + offset); }
    //绝对坐标
            ...
            for (int neg = -1; neg < 2; neg += 2) {
                for (int l = 1; l < 6; ++l) {
                    ...
                    if (side != BOARD_VAL::EMP) {
                        ...
                        auto [x, y] = Mappings[k](xn + offset, yn + offset,
                                                  size + offset * 2);                        //转成bit坐标
